...
function blogGet($idx)
{
    ...
    if ($stmt->execute()) {
     ...
     {
         return GetOtherKeysValuesInJson(array("id","title","img","description","author","created_time"),array($row["id"],$row["title"],$row["img"],$row["description"],$row["author"],$row["created_time"]));
     }
    }
...
{
    global $conn;
    $query = 'Select * from blog order by created_time';
    $stmt = $conn->prepare($query);
    if ($stmt->execute()) {
     $result = $stmt->get_result();
     $MainRes = array();
     ...
     while($row = $result->fetch_assoc())
     {
         $Json1 = GetOtherKeysValuesInJson(array("id","title","img","description","author","created_time"),array($row["id"],$row["title"],$row["img"],$row["description"],$row["author"],$row["created_time"]));
...
function Subscribe($Email)
{
    ...
    global $conn;
    $query = 'insert into subscribers(id,email,user_id) values(null,?,-1)';
    $stmt = $conn->prepare($query);
...
function Contact($Email,$Suject,$Message)
{
    global $conn;
    $query = 'insert into contact values(null,?,?,?)';
    $stmt = $conn->prepare($query);
    $stmt->bind_param("sss",$Email ,$Suject, $Message);
